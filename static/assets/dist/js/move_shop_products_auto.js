...
function bindEvents() {
  ...
  $(document).on("change", ".manual-match-select", function () {
    ...
    if (selectedValue === "") {
      ...
      const selectedMatch = sourceShopProducts.find((p) => p.id === shopProductId) || null;
      ...
      return `
        <tr>
          <td class="text-center">
            <input type="checkbox" class="entry-check" data-index="${index}" ${canMove ? "checked" : "disabled"}>
          </td>
...
function renderMatches(entry) {
  ...
  if (entry.status === "ambiguo") {
    ...
    const allOptions = sourceShopProducts
      .map(
        (match) =>
          `<option value="shop_${match.id}">${escapeHtml(match.displayName)} (${escapeHtml(
            match.modelBrand
...
function renderStatus(entry) {
  ...
  if (entry.status === "no_encontrado") {
    return '<span class="badge badge-danger">No encontrado en tienda origen</span>';
  }
...
async function crearMovimientos() {
  ...
  try {
    ...
    payloadProducts.forEach((item) => {
      const matched = sourceShopProducts.find((shopProduct) => shopProduct.id === item.shop_product);
      if (matched) {
        matched.currentQuantity = Math.max(0, matched.currentQuantity - item.quantity);
      }
    });
